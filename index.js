...
function profilListesi(feno) {
  /*kod*/
  const newDizi = [...feno];
  const justProfileName = [];
  for (let i = 0; i < newDizi.length; i++) {
    justProfileName.push(newDizi[i].profile);
  }
  return justProfileName;
}
...
function fenomenEkle(dizi, no, ad, takip, gönderi, yer) {
  /*kod*/
  const newArr = [...dizi];
  const newObj = {
    number: no,
...
function enFenomenler(dizi) {
  /*kod*/
  const millionsFollowers = [];
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i].followers > 100000000) {
      millionsFollowers.push(dizi[i].profile);
    }
  }
  return millionsFollowers;
}
...
function fenomenGonderimSayisi(dizi, feno) {
  /*kod*/
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i].profile === feno) {
      return dizi[i].posts;
    }
  }
}
...
function platformaGoreCokGonderiYapanFenomen(dizi, yer) {
  /*kod*/
  let maxPost = 0;
  let maxPostGönderen = "";
  for (let i = 0; i < dizi.length; i++) {
    if (
      dizi[i].platform === yer &&
      dizi[i].posts > maxPost &&
      dizi[i].posts !== "NA"
    ) {
      maxPost = dizi[i].posts;
      maxPostGönderen = dizi[i].profile;
    }
  }
  return maxPostGönderen;
}
